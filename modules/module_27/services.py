...
def calculate_heart_score(
    ...
    HEART Score (0-10). Each component scored 0-2:
      H = History (suspicious → 2, moderately → 1, slightly → 0)
      E = ECG (ST-deviation → 2, non-specific → 1, normal → 0)
    ...
    # T — Troponin (normal < 0.04)
    if troponin > 0.12:     # > 3x normal
        score += 2
    elif troponin > 0.04:   # 1-3x normal
        score += 1
...
def calculate_timi_score(
    ...
    """
    TIMI Risk Score for UA/NSTEMI (0-7). Each item = 1 point:
      1. Age ≥ 65
      2. ≥ 3 CAD risk factors
      3. Known CAD (stenosis ≥ 50%)
      4. ASA use in past 7 days
      5. ≥ 2 angina events in 24h
      6. ST deviation on ECG
      7. Elevated cardiac markers (troponin > 0.04)
    """
    score = 0
    if age >= 65:
        score += 1
    ...
    ecg_lower = ecg_rhythm.lower()
    if "st-elevation" in ecg_lower or "st-depression" in ecg_lower:
        score += 1
...
def calculate_grace_score(
    ...
    """
    Simplified GRACE Score (0-372). Combines weighted risk factors.
    Real GRACE uses a lookup table — here we use clinically approximate weights.
    """
    score = 0
    ...
    # Systolic BP (inverse — lower is worse) (0-58)
    if systolic_bp < 80:
        score += 58
    ...
    # Cardiac arrest on admission (0-39)
    if cardiac_arrest:
        score += 39
...
def determine_hf_severity(bnp_level: float, killip_class: int) -> str:
    """Heart failure severity based on BNP level and Killip class."""
    if killip_class >= 4 or bnp_level > 900:
        return "Severe"
    elif killip_class >= 3 or bnp_level > 400:
        return "Moderate"
    elif killip_class >= 2 or bnp_level > 100:
        return "Mild"
    return "None"
...
def assess_hemodynamics(systolic_bp: int, diastolic_bp: int, heart_rate: int) -> str:
    ...
    Returns: "Stable" | "Unstable" | "Critical"
    """
    if systolic_bp < 70 or heart_rate < 40 or heart_rate > 150:
        return "Critical"
    if systolic_bp < 90 or systolic_bp > 180 or heart_rate < 50 or heart_rate > 120:
        return "Unstable"
    if diastolic_bp > 120:
        return "Unstable"
    return "Stable"
...
async def process_cardiac_telemetry(payload: CardiacParameterCreate) -> Dict[str, Any]:
    ...
    2. Calculate HEART / TIMI / GRACE scores
    3. Assess hemodynamics
    4. Check STEMI & HF severity
    5. Persist everything to MongoDB
    6. Return full assessment
    """
    # 1. Arrhythmia detection
    alerts = detect_arrhythmias(
        payload.patient_id,
        payload.ecg_rhythm,
        payload.heart_rate,
        payload.troponin_level,
    )
    stemi_alert = any(a.type == "STEMI" for a in alerts)

    # 2. Scoring
    heart = calculate_heart_score(
        payload.history_notes or "",
        payload.ecg_rhythm,
        payload.age,
        payload.risk_factors,
        payload.troponin_level,
    )
    timi = calculate_timi_score(
        payload.age,
        payload.risk_factors,
        payload.ecg_rhythm,
        payload.troponin_level,
        payload.aspirin_use,
        payload.known_stenosis,
        payload.angina_episodes_24h,
    )
    grace = calculate_grace_score(
        payload.age,
        payload.heart_rate,
        payload.systolic_bp,
        payload.creatinine,
        payload.cardiac_arrest_on_admission,
        payload.ecg_rhythm,
        payload.troponin_level,
        payload.killip_class,
    )
    ...
    scoring = ScoringResult(
        ...
        stemi_flag=stemi_alert,
        hf_severity=hf_severity,
    )

    # 3. Hemodynamics
    hemo_status = assess_hemodynamics(
        payload.systolic_bp,
    ...
    telemetry_doc = {
        ...
        "ecg_rhythm": payload.ecg_rhythm,
        "heart_rate": payload.heart_rate,
        "systolic_bp": payload.systolic_bp,
        "diastolic_bp": payload.diastolic_bp,
        "troponin_level": payload.troponin_level,
        "bnp_level": payload.bnp_level,
        "creatinine": payload.creatinine,
        "echo_findings": payload.echo_findings,
    ...
    for a in alerts:
        ...
        alert_id = await save_arrhythmia_alert(alert_doc)
        a.alert_id = alert_id
        alert_dicts.append(a)

    # 6. Persist hemodynamic log
    await save_hemodynamic_log({
        "patient_id": payload.patient_id,
        "systolic_bp": payload.systolic_bp,
        "diastolic_bp": payload.diastolic_bp,
        "heart_rate": payload.heart_rate,
